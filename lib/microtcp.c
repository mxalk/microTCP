...
microtcp_connect (microtcp_sock_t *socket, const struct sockaddr *address,
                  socklen_t address_len)
{
  if (socket->state != BIND && socket->state != UNKNOWN) return STATE_ERROR;
...
printf("got: #%d\n", htonl(hrecv->ack_number));
    if (ntohl(hrecv->ack_number) > socket->seq_number) {
	/* Normal */
	  total_sent += ntohl(hrecv->ack_number) - socket->seq_number - sizeof(microtcp_header_t);
      socket->seq_number = ntohl(hrecv->ack_number);
	  socket->curr_win_size = ntohs(hrecv->window);
	  if (socket->cong_alg == SLOW_START) socket->cwnd += MICROTCP_MSS;
	} else if (socket->seq_number + MICROTCP_MSS + sizeof(microtcp_header_t) < MICROTCP_MSS + sizeof(microtcp_header_t) && ntohl(hrecv->ack_number) <= socket->seq_number + MICROTCP_MSS + sizeof(microtcp_header_t)) {
	/* Wrap around */
	  total_sent += (MICROTCP_MSS - (socket->seq_number + MICROTCP_MSS)) + ntohl(hrecv->ack_number) - sizeof(microtcp_header_t);
	  socket->seq_number = ntohl(hrecv->ack_number);
	  socket->curr_win_size = ntohs(hrecv->window);
	  if (socket->cong_alg == SLOW_START) socket->cwnd += MICROTCP_MSS;
	} else {
...
microtcp_recv (microtcp_sock_t *socket, void *buffer, size_t length, int flags)
{
  ...
  if (recv == SHUTDOWN_CODE) microtcp_shutdown(socket, 0);
