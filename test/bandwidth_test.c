...
print_statistics (ssize_t received, struct timespec start, struct timespec end)
{
  ...
  /*Should be 1000*1000 for the result to be according to LINUX file size*/
...
server_microtcp (uint16_t listen_port, const char *file)
{
  ...
  if (sock.sd == -1) {
    perror ("Opening micro_TCP socket");
    free (buffer);
    fclose (fp);
    return -EXIT_FAILURE;
  }
